...
from modules.pretrain import PreTrainer
from modules.dual_train import DualNERTrainer
from modules.ddpm_train import DDPMTrainer
...
DATA_PATH = {
    'twitter15': {
                ...
                'flexmatch': 'data/NER_data/twitter2015/unlabeled.problems.txt',
                'mean_teacher': 'data/NER_data/twitter2015/unlabeled.txt',
    ...
    'twitter17': {
                ...
                'flexmatch': 'data/NER_data/twitter2017/unlabeled.txt',
                'mean_teacher': 'data/NER_data/twitter2017/unlabeled.txt',
...
def main():
    ...
    parser.add_argument('--label_hidden_dim', default=128, type=int, help="The hidden dimensions of label features.")
    ...
    parser.add_argument('--base_models_path', default=None, type=str, help="Path to base MNER models.")
    ...
    parser.add_argument('--do_flexmatch_training', action='store_true')
    parser.add_argument('--do_mean_teacher_training', action='store_true')
    ...
    # unlabeled_dataset = dataset_class(processor, transform, imgs_path, aux_imgs_path, args.max_seq_len, args.max_char_len, mode="flexmatch", aux_size=args.aux_size, rcnn_imgs_path=rcnn_imgs_path, rcnn_size=args.rcnn_size)
    # unlabeled_dataset = dataset_class(processor, transform, imgs_path, aux_imgs_path, args.max_seq_len, args.max_char_len, mode="mean_teacher", aux_size=args.aux_size, rcnn_imgs_path=rcnn_imgs_path, rcnn_size=args.rcnn_size)
    ...
    if args.do_pretrain:
        ner_model = HMNeTNERModel(len(label_list), args)
        trainer = PreTrainer(train_data=train_dataloader_unlabeled, val_data=val_dataloader_unlabeled, test_data=test_dataloader_unlabeled, ner_model=ner_model, ner_model_name="hvpnet",
        ...
        ner_model = UnimoCRFModel(len(label_list), args)
        trainer = PreTrainer(train_data=train_dataloader_unlabeled, val_data=val_dataloader_unlabeled, test_data=test_dataloader_unlabeled, ner_model=ner_model, 
                      label_map=label_mapping, args=args, logger=logger, writer=writer)
        trainer.train()
        args.load_path = os.path.join(args.save_path, "best_model")
        trainer.test()
        os.rename("best_model.pth", "mkgformer.pth")

    if args.do_train:
        # Fine-tune the HVPNeT model on the labeled data #
        hvpnet_model_old = HMNeTNERModel(len(label_list), args)
        hvpnet_model_new = HMNeTNERModel(len(label_list), args)
        hvpnet_model_old.load_state_dict(torch.load(os.path.join(args.save_path, "hvpnet.pth")))
        hvpnet_model_new.load_state_dict(torch.load(os.path.join(args.save_path, "hvpnet.pth")))
        hvpnet_model_old.to(args.device)
        hvpnet_model_new.to(args.device)
        hvpnet_trainer = DualNERTrainer(train_data=train_dataloader_labeled, val_data=val_dataloader_labeled, test_data=test_dataloader_labeled, ner_base_model="hvpnet",
                                 ner_model_old=hvpnet_model_old, ner_model_new=hvpnet_model_new, label_map=label_mapping, args=args, logger=logger, writer=writer)
        hvpnet_trainer.train()
        args.load_path = os.path.join(args.save_path, "best_model")
        hvpnet_trainer.test()
        os.rename("best_model_old.pth", "hvpnet_old.pth")
        os.rename("best_model_new.pth", "hvpnet_new.pth")
        # Fine-tune the MKGFormer model on the labeled data #
        mkg_model_old = UnimoCRFModel(len(label_list), args)
        mkg_model_new = UnimoCRFModel(len(label_list), args)
        mkg_model_old.load_state_dict(torch.load(os.path.join(args.save_path, "mkgformer.pth")))
        mkg_model_new.load_state_dict(torch.load(os.path.join(args.save_path, "mkgformer.pth")))
        mkg_model_old.to(args.device)
        mkg_model_new.to(args.device)
        mkg_trainer = DualNERTrainer(train_data=train_dataloader_labeled, val_data=val_dataloader_labeled, test_data=test_dataloader_labeled, ner_base_model="mkgformer",
                                 ner_model_old=mkg_model_old, ner_model_new=mkg_model_new, label_map=label_mapping, args=args, logger=logger, writer=writer)
        mkg_trainer.train()
        args.load_path = os.path.join(args.save_path, "best_model")
        mkg_trainer.test()
        os.rename("best_model_old.pth", "mkgformer_old.pth")
        os.rename("best_model_new.pth", "mkgformer_new.pth")
    
    if args.do_adv_train:
        forward_net = DiffusionModel(args, len(label_list), clstm_path, vt_model_name="mkgformer", vt_model_dir="new")
        forward_net.to(args.device)
        reverse_net = DiffusionModel(args, len(label_list), clstm_path, vt_model_name="mkgformer", vt_model_dir="old")
    ...
    if args.predict:
        forward_net = DiffusionModel(args, len(label_list), clstm_path, vt_model_name="hvpnet", vt_model_dir="new")
        forward_net.to(args.device)
        reverse_net = DiffusionModel(args, len(label_list), clstm_path, vt_model_name="hvpnet", vt_model_dir="old")
        reverse_net.to(args.device)
        ddpm_trainer = DDPMTrainer(train_data=train_dataloader_labeled, val_data=val_dataloader_labeled, test_data=test_dataloader_labeled, unlabeled_data=val_dataloader_unlabeled,
                                         forward_net=forward_net, reverse_net=reverse_net, label_map=label_mapping, args=args, logger=logger, writer=writer)
        args.load_path = os.path.join(args.save_path, "best_")
    ...
    # if args.do_flexmatch_training:
    #     forward_net = DiffusionModel(args, len(label_list), clstm_path, vt_model_name="mkgformer", vt_model_dir="new")
    #     forward_net.to(args.device)
    #     reverse_net = DiffusionModel(args, len(label_list), clstm_path, vt_model_name="mkgformer", vt_model_dir="old")
    #     reverse_net.to(args.device)
    #     args.load_path = os.path.join(args.save_path, "best_")
    #     semi_ddpm_trainer = DDPMTrainer(train_data=train_dataloader_labeled, val_data=val_dataloader_labeled, test_data=test_dataloader_labeled, unlabeled_data=train_dataloader_unlabeled, 
    #                                      forward_net=forward_net, reverse_net=reverse_net, label_map=label_mapping, args=args, logger=logger, writer=writer)
    #     semi_ddpm_trainer.train_flexmatch()
    #     semi_ddpm_trainer.test()

    # if args.do_mean_teacher_training:
    #     forward_net = DiffusionModel(args, len(label_list), clstm_path, vt_model_name="mkgformer", vt_model_dir="new")
    #     forward_net.to(args.device)
    #     reverse_net = DiffusionModel(args, len(label_list), clstm_path, vt_model_name="mkgformer", vt_model_dir="old")
    #     reverse_net.to(args.device)
    #     args.load_path = os.path.join(args.save_path, "best_")
    #     semi_ddpm_trainer = DDPMTrainer(train_data=train_dataloader_labeled, val_data=val_dataloader_labeled, test_data=test_dataloader_labeled, unlabeled_data=test_dataloader_unlabeled, 
    #                                      forward_net=forward_net, reverse_net=reverse_net, label_map=label_mapping, args=args, logger=logger, writer=writer)
    #     semi_ddpm_trainer.train_mean_teacher()
    #     semi_ddpm_trainer.test()
